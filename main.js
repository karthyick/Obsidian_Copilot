...
${s}`}convertToLLMMessages(e,t,r){let s=[];for(let i of e)s.push({role:i.role,content:i.content});return s}async buildMessages(e,t,r){let s=[];for(let o of e)s.push({role:o.role,content:o.content});let i=await this.buildContextMessage(t,r);return s.push({role:"user",content:i}),s}async getContextSummary(){let e=await this.noteController.getNoteContext();if(!e)return null;let t=[];if(t.push(`\u{1F4C4} ${e.basename}`),e.selection){let s=e.selectionRange?` (L${e.selectionRange.startLine+1}-${e.selectionRange.endLine+1})`:"";t.push(`\u{1F4DD} Selection${s}`)}let r=e.content.split(/\s+/).length;return t.push(`${r} words`),t.join(" \u2022 ")}hasNoteContext(){return this.noteController.hasActiveNote()}estimateTokens(e){return Math.ceil(e.length/te.CHARS_PER_TOKEN)}getMaxMessageLength(){return te.MAX_NOTE_CHARS}};te.MAX_NOTE_TOKENS=8e3,te.CHARS_PER_TOKEN=4,te.MAX_NOTE_CHARS=te.MAX_NOTE_TOKENS*te.CHARS_PER_TOKEN;var jr=te;var C=require("obsidian");var ve=class ve{extractMermaidBlocks(e){let t=[],r=new RegExp(ve.MERMAID_PATTERN.source,"g"),s;for(;(s=r.exec(e))!==null;)t.push({code:s[1].trim(),startIndex:s.index,endIndex:s.index+s[0].length});return t}hasMermaid(e){return ve.MERMAID_PATTERN.test(e)}validateMermaid(e){let t=e.trim();if(!t)return{valid:!1,diagramType:null,error:"Empty Mermaid code"};let r=t.split(`
`)[0].trim().toLowerCase(),s=null;for(let d of ve.DIAGRAM_TYPES)if(r.startsWith(d.toLowerCase())){s=d;break}if(!s)return{valid:!1,diagramType:null,error:`Unknown diagram type. First line: "${r}"`};let i=(t.match(/\[/g)||[]).length,o=(t.match(/\]/g)||[]).length,a=(t.match(/\(/g)||[]).length,c=(t.match(/\)/g)||[]).length,l=(t.match(/\{/g)||[]).length,u=(t.match(/\}/g)||[]).length;return i!==o?{valid:!1,diagramType:s,error:"Mismatched brackets []"}:a!==c?{valid:!1,diagramType:s,error:"Mismatched parentheses ()"}:l!==u?{valid:!1,diagramType:s,error:"Mismatched braces {}"}:{valid:!0,diagramType:s,error:null}}sanitizeMermaidCode(e){let t=e.split(`
...
`)[0].trim().toLowerCase();for(let r of ve.DIAGRAM_TYPES)if(t.startsWith(r.toLowerCase()))return r;return null}createFlowchartTemplate(e,t,r){let s=["flowchart TD"];for(let i of t)s.push(`    ${i.id}[${i.label}]`);for(let i of r){let o=i.label?`|${i.label}|`:"";s.push(`    ${i.from} -->${o} ${i.to}`)}return s.join(`
...
`)}replaceMermaidWithPlaceholders(e){let t=this.extractMermaidBlocks(e),r=e,s=0;for(let i=0;i<t.length;i++){let o=t[i],a=`[MERMAID_DIAGRAM_${i}]`,c=o.endIndex-o.startIndex;r=r.substring(0,o.startIndex-s)+a+r.substring(o.endIndex-s),s+=c-a.length}return{text:r,blocks:t}}static getSupportedDiagramTypes(){return[...ve.DIAGRAM_TYPES]}};ve.MERMAID_PATTERN=/```mermaid\n([\s\S]*?)```/g,ve.DIAGRAM_TYPES=["flowchart","graph","sequenceDiagram","classDiagram","stateDiagram","stateDiagram-v2","erDiagram","gantt","pie","journey","gitGraph","mindmap","timeline","quadrantChart","xychart-beta","block-beta","sankey-beta"];var Kr=ve;var fC=`# PYRAMID - Minto Pyramid Principle Document Transformer
...
Identify BLACK HOLES (knowledge gaps).
Output as a replace_all edit command to replace the document content.`,vC=`# CRINGE - Critical Review & Intelligent Negative Gauntlet Engine
...
DO NOT ask any questions - work with what's there.`;function Qd(n){if(n.size===0)return"";let e=[];return n.has("pyramid")&&e.push(fC),n.has("coin")&&e.push(gC),n.has("developer")&&e.push(yC),n.has("business")&&e.push(SC),n.has("management")&&e.push(EC),n.has("cosmos")&&e.push(TC),n.has("cringe")&&e.push(vC),`
