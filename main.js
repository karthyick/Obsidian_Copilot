...
${s}`}convertToLLMMessages(e,t,r){let s=[];for(let i of e)s.push({role:i.role,content:i.content});return s}async buildMessages(e,t,r){let s=[];for(let o of e)s.push({role:o.role,content:o.content});let i=await this.buildContextMessage(t,r);return s.push({role:"user",content:i}),s}async getContextSummary(){let e=await this.noteController.getNoteContext();if(!e)return null;let t=[];if(t.push(`\u{1F4C4} ${e.basename}`),e.selection){let s=e.selectionRange?` (L${e.selectionRange.startLine+1}-${e.selectionRange.endLine+1})`:"";t.push(`\u{1F4DD} Selection${s}`)}let r=e.content.split(/\s+/).length;return t.push(`${r} words`),t.join(" \u2022 ")}hasNoteContext(){return this.noteController.hasActiveNote()}estimateTokens(e){return Math.ceil(e.length/te.CHARS_PER_TOKEN)}getMaxMessageLength(){return te.MAX_NOTE_CHARS}};te.MAX_NOTE_TOKENS=8e3,te.CHARS_PER_TOKEN=4,te.MAX_NOTE_CHARS=te.MAX_NOTE_TOKENS*te.CHARS_PER_TOKEN;var jr=te;var v=require("obsidian");var Ce=class Ce{extractMermaidBlocks(e){let t=[],r=new RegExp(Ce.MERMAID_PATTERN.source,"g"),s;for(;(s=r.exec(e))!==null;)t.push({code:s[1].trim(),startIndex:s.index,endIndex:s.index+s[0].length});return t}hasMermaid(e){return Ce.MERMAID_PATTERN.test(e)}validateMermaid(e){let t=e.trim();if(!t)return{valid:!1,diagramType:null,error:"Empty Mermaid code"};let r=t.split(`
`)[0].trim().toLowerCase(),s=null;for(let d of Ce.DIAGRAM_TYPES)if(r.startsWith(d.toLowerCase())){s=d;break}if(!s)return{valid:!1,diagramType:null,error:`Unknown diagram type. First line: "${r}"`};let i=(t.match(/\[/g)||[]).length,o=(t.match(/\]/g)||[]).length,a=(t.match(/\(/g)||[]).length,c=(t.match(/\)/g)||[]).length,l=(t.match(/\{/g)||[]).length,u=(t.match(/\}/g)||[]).length;return i!==o?{valid:!1,diagramType:s,error:"Mismatched brackets []"}:a!==c?{valid:!1,diagramType:s,error:"Mismatched parentheses ()"}:l!==u?{valid:!1,diagramType:s,error:"Mismatched braces {}"}:{valid:!0,diagramType:s,error:null}}sanitizeMermaidCode(e){let t=e.split(`
...
`)[0].trim().toLowerCase();for(let r of Ce.DIAGRAM_TYPES)if(t.startsWith(r.toLowerCase()))return r;return null}createFlowchartTemplate(e,t,r){let s=["flowchart TD"];for(let i of t)s.push(`    ${i.id}[${i.label}]`);for(let i of r){let o=i.label?`|${i.label}|`:"";s.push(`    ${i.from} -->${o} ${i.to}`)}return s.join(`
...
`)}replaceMermaidWithPlaceholders(e){let t=this.extractMermaidBlocks(e),r=e,s=0;for(let i=0;i<t.length;i++){let o=t[i],a=`[MERMAID_DIAGRAM_${i}]`,c=o.endIndex-o.startIndex;r=r.substring(0,o.startIndex-s)+a+r.substring(o.endIndex-s),s+=c-a.length}return{text:r,blocks:t}}static getSupportedDiagramTypes(){return[...Ce.DIAGRAM_TYPES]}};Ce.MERMAID_PATTERN=/```mermaid\n([\s\S]*?)```/g,Ce.DIAGRAM_TYPES=["flowchart","graph","sequenceDiagram","classDiagram","stateDiagram","stateDiagram-v2","erDiagram","gantt","pie","journey","gitGraph","mindmap","timeline","quadrantChart","xychart-beta","block-beta","sankey-beta"];var Kr=Ce;var fv=`# PYRAMID - Minto Pyramid Principle Document Transformer
...
Output as a replace_all edit command to replace the document content.`,gv=`# COIN - Context-Observation-Insight-Next Steps Document Transformer
...
Keep EVERY piece of information from the original.
Output as a replace_all edit command to replace the document content.`,yv=`# DEVELOPER - Technical Documentation Transformer
...
Output as a replace_all edit command to replace the document content.`,Sv=`# BUSINESS - Executive Business Communication Transformer
...
Quantify EVERYTHING possible.
Keep EVERY piece of information from the original.
Output as a replace_all edit command to replace the document content.`,Ev=`# MANAGEMENT - Executive Briefing Transformer
...
Keep EVERY piece of information from the original.
Output as a replace_all edit command to replace the document content.`,Tv=`# COSMOS - Knowledge Constellation Architect
...
Identify BLACK HOLES (knowledge gaps).
Output as a replace_all edit command to replace the document content.`,Cv=`# CRINGE - Critical Review & Intelligent Negative Gauntlet Engine
...
DO NOT ask any questions - work with what's there.`;function Qd(n){if(n.size===0)return"";let e=[];return n.has("pyramid")&&e.push(fv),n.has("coin")&&e.push(gv),n.has("developer")&&e.push(yv),n.has("business")&&e.push(Sv),n.has("management")&&e.push(Ev),n.has("cosmos")&&e.push(Tv),n.has("cringe")&&e.push(Cv),`
...
`),t.join(`
`)}createChatMessage(t,r){return{id:crypto.randomUUID(),role:t,content:r,timestamp:new Date}}renderMessage(t,r=!1){let s=this.messagesContainer.createDiv({cls:`ai-assistant-message ai-assistant-message-${t.role}`}),i=s.createDiv({cls:"ai-assistant-avatar"});t.role==="assistant"?(0,v.setIcon)(i,"bot"):(0,v.setIcon)(i,"user");let o=s.createDiv({cls:"ai-assistant-message-wrapper"}),c=o.createDiv({cls:"ai-assistant-bubble"}).createDiv({cls:"ai-assistant-message-content"});if(t.content)this.updateMessageContent(c,t.content);else if(r){let d=c.createDiv({cls:"ai-assistant-typing-modern"});d.createSpan({cls:"ai-assistant-typing-dot"}),d.createSpan({cls:"ai-assistant-typing-dot"}),d.createSpan({cls:"ai-assistant-typing-dot"})}let l=o.createDiv({cls:"ai-assistant-message-meta-row"});if(l.createDiv({cls:"ai-assistant-message-meta"}).createSpan({text:t.timestamp.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}),t.role==="user"&&t.content){let m=l.createDiv({cls:"ai-assistant-user-actions"}).createEl("button",{cls:"ai-assistant-action-btn-small",attr:{"aria-label":"Copy message"}});(0,v.setIcon)(m,"copy"),m.addEventListener("click",()=>{(async()=>(await navigator.clipboard.writeText(t.content),(0,v.setIcon)(m,"check"),new v.Notice("Copied to clipboard"),setTimeout(()=>(0,v.setIcon)(m,"copy"),2e3)))()})}return this.scrollToBottom(),s}async updateMessageContent(t,r){t.empty();let s=this.editProtocol.getDisplayText(r);await v.MarkdownRenderer.render(this.app,s,t,"",this),t.querySelectorAll("pre").forEach(o=>{let a=o.createEl("button",{cls:"ai-assistant-copy-code",attr:{"aria-label":"Copy code"}});(0,v.setIcon)(a,"copy"),a.addEventListener("click",()=>{var l,u;let c=(u=(l=o.querySelector("code"))==null?void 0:l.textContent)!=null?u:"";navigator.clipboard.writeText(c).then(()=>{(0,v.setIcon)(a,"check"),setTimeout(()=>(0,v.setIcon)(a,"copy"),2e3)})})})}addMessageActions(t,r){let s=t.querySelector(".ai-assistant-message-wrapper");if(!s)return;let i=s.createDiv({cls:"ai-assistant-message-actions"}),o=i.createEl("button",{cls:"ai-assistant-action-btn-modern",attr:{"aria-label":"Copy"}});(0,v.setIcon)(o,"copy"),o.addEventListener("click",()=>{let l=this.editProtocol.getDisplayText(r.content);navigator.clipboard.writeText(l).then(()=>{new v.Notice("Copied to clipboard")})});let a=i.createEl("button",{cls:"ai-assistant-action-btn-modern",attr:{"aria-label":"Insert at cursor"}});if((0,v.setIcon)(a,"file-input"),a.addEventListener("click",()=>{let l=this.editProtocol.getDisplayText(r.content);this.plugin.noteController.insertAtCursor(l)?new v.Notice("Inserted at cursor"):new v.Notice("No active note")}),r.editCommands&&r.editCommands.length>0){let l=i.createEl("button",{cls:"ai-assistant-action-btn-modern ai-assistant-action-apply",attr:{"aria-label":"Apply edit"}});r.applied?((0,v.setIcon)(l,"check-circle"),l.disabled=!0,l.addClass("ai-assistant-action-applied")):((0,v.setIcon)(l,"edit"),l.addEventListener("click",()=>{let u=this.editProtocol.executeCommands(r.editCommands);if(u.every(m=>m.success))r.applied=!0,(0,v.setIcon)(l,"check-circle"),l.disabled=!0,l.addClass("ai-assistant-action-applied"),new v.Notice("Edit applied successfully");else{let m=u.find(p=>!p.success);new v.Notice(`Edit failed: ${m==null?void 0:m.message}`)}}))}if(r.mermaidBlocks&&r.mermaidBlocks.length>0){let l=i.createEl("button",{cls:"ai-assistant-action-btn-modern",attr:{"aria-label":"Insert diagram"}});(0,v.setIcon)(l,"git-branch"),l.addEventListener("click",()=>{for(let u of r.mermaidBlocks)this.plugin.noteController.insertMermaid(u)?new v.Notice("Diagram inserted"):new v.Notice("No active note")})}let c=i.createEl("button",{cls:"ai-assistant-action-btn-modern ai-assistant-action-retry",attr:{"aria-label":"Retry"}});(0,v.setIcon)(c,"refresh-cw"),c.addEventListener("click",()=>{this.retryMessage(r,t)})}async retryMessage(t,r){let s=this.chatHistory.findIndex(u=>u.id===t.id);if(s===-1||s===0){new v.Notice("Cannot retry this message");return}let i=this.chatHistory[s-1];if(!i||i.role!=="user"){new v.Notice("Cannot find original request");return}let o=i.content;r.remove(),this.chatHistory.splice(s,1),this.setLoading(!0);let a=this.createChatMessage("assistant","");this.chatHistory.push(a);let c=this.renderMessage(a,!0),l=c.querySelector(".ai-assistant-message-content");try{let d=this.chatHistory.slice(0,-1).slice(-10),m=await this.plugin.contextBuilder.buildMessages(d,o,this.contextToggle.checked),p=this.plugin.contextBuilder.buildSystemPrompt(this.plugin.settings.systemPrompt),g="";if(this.plugin.settings.streamResponses){let y=this.plugin.llmService.sendMessageStream(m,p);for await(let S of y)g+=S,a.content=g,await this.updateMessageContent(l,g),this.scrollToBottom()}else g=await this.plugin.llmService.sendMessage(m,p),a.content=g,await this.updateMessageContent(l,g);a.editCommands=this.editProtocol.parseEditCommands(g),a.mermaidBlocks=this.mermaidHandler.extractMermaidCode(g),this.addMessageActions(c,a),new v.Notice("Response regenerated")}catch(u){let d=u instanceof Error?u.message:String(u);a.content=`**Error regenerating response:** ${d}`,await this.updateMessageContent(l,a.content),this.addMessageActions(c,a)}finally{this.setLoading(!1)}}setLoading(t){this.isLoading=t,this.sendButton.disabled=t,this.inputTextarea.disabled=t,t?this.sendButton.addClass("ai-assistant-loading"):this.sendButton.removeClass("ai-assistant-loading")}scrollToBottom(){this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight}stripMarkdownWrapper(t){let r=t.trim(),s=/^```(?:markdown)?\s*\n([\s\S]*?)\n```\s*$/,i=r.match(s);return i?i[1]:t}clearChat(){this.chatHistory=[],this.referencedNotes=[],this.messagesContainer.empty(),this.renderReferencedNotes(),this.addWelcomeMessage(),new v.Notice("Chat cleared")}getChatHistory(){return[...this.chatHistory]}focusInput(){var t;(t=this.inputTextarea)==null||t.focus()}async setInputAndSend(t,r=!1){this.inputTextarea&&(this.inputTextarea.value=t,r?await this.sendMessage():this.inputTextarea.focus())}};var Yr=class extends Qr.Plugin{async onload(){await this.loadSettings(),this.noteController=new zr(this.app),this.llmService=new Vr(this.settings),this.contextBuilder=new jr(this.noteController,()=>this.settings.excludedNotes),this.registerView(_e,e=>new Gt(e,this)),this.addRibbonIcon("bot","AI assistant",()=>{this.activateView()}),this.registerCommands(),this.addSettingTab(new $t(this.app,this)),this.checkSettings()}onunload(){}registerCommands(){this.addCommand({id:"open-ai-chat",name:"Open AI assistant",callback:()=>{this.activateView()}}),this.addCommand({id:"send-note-to-ai",name:"Send note to AI",editorCallback:async()=>{await this.activateView();let e=await this.noteController.getActiveNoteContent(),t=this.getChatView();e&&t&&await t.setInputAndSend("Please analyze and summarize this note.",!0)}}),this.addCommand({id:"ai-edit-selection",name:"AI edit selection",editorCallback:async()=>{let e=this.noteController.getSelection();if(!e){new Qr.Notice("Please select some text first");return}await this.activateView();let t=this.getChatView();t&&await t.setInputAndSend(`Please improve or edit the following selected text:
...
"${e}"`,!0):await t.setInputAndSend("Please expand on the main topics in this note, adding more detail and context where appropriate.",!0))}}),this.addCommand({id:"ai-clear-chat",name:"Clear AI chat",callback:()=>{let e=this.getChatView();e&&e.clearChat()}})}async activateView(){let{workspace:e}=this.app,t=null,r=e.getLeavesOfType(_e);if(r.length>0?t=r[0]:(t=e.getRightLeaf(!1),t&&await t.setViewState({type:_e,active:!0})),t){e.revealLeaf(t);let s=this.getChatView();s&&s.focusInput()}}async loadSettings(){this.settings=Object.assign({},Ni,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}reinitializeLLMService(){this.llmService.reinitialize(this.settings)}async testConnection(){let e=es(this.settings);return e.valid?(this.reinitializeLLMService(),await this.llmService.testConnection()):{success:!1,message:e.errors.join(", ")}}async testProviderConnection(e){return this.reinitializeLLMService(),await this.llmService.testProviderConnection(e)}checkSettings(){es(this.settings).valid}getChatView(){let e=this.app.workspace.getLeavesOfType(_e);if(e.length===0)return null;let t=e[0].view;return t instanceof Gt?t:null}getCurrentProviderName(){return this.llmService.getCurrentProviderName()}};
